...
const helpinfo = require('./helpdata')
...
class RPC extends RPCBase {
  ...
  async help(args, _help) {
    if (args.length === 0)
      return Object.keys(helpinfo).reduce((o, k, i) => {
        if(i) o+='\n'
        o += `== ${k} ==\n`
        Object.keys(helpinfo[k]).forEach(c => {
          o += `${helpinfo[k][c].signature}\n`
        })
        return o
      },'')
  ...
  async getNetworkInfo(args, help) {
    if (help || args.length !== 0) {
      const info = helpinfo.Network.getnetworkinfo;
      throw new RPCError(errs.MISC_ERROR, this.helpDetail(info));
